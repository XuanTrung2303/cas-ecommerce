    ...
    <script>
                ...
                <div class="grid grid-cols-1 md:grid-cols-5 gap-3">
                    ...
                    <div>
                        ...
                        <select name="color_id[]"
                            class="w-full bg-slate-600 border border-gray-700 rounded py-0.5 focus:outline-none text-white" required>
                           ${colorOptions}
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="mrp[]" placeholder="Enter MRP"
                            class="w-full bg-transparent border border-gray-700 rounded py-0.5 px-2 focus:outline-none text-white" required>
                    </div>
        ...
        const setImagePreview = (r, e, isAdd = true) => {
            if (e.target.files.length > 0) {
                r.setAttribute('onchange', 'setImagePreview(this, event, flase)');
                r.nextElementSibling.src = URL.createObjectURL(e.target.files[0]);

                let span = `<span onclick="deleteImage(this)" class="absolute top-1 right-1 cursor-pointer w-7 h-7 flex items-center justify-center bg-white hover:bg-red-500 bg-opacity-25 hover:opacity-100 text-red-500 hover:text-white duration-300 shadow rounded-full">
...
@section('body_content')
    ...
    <form action="{{ route('dpanel.product.update', $data->id) }}" method="post" enctype="multipart/form-data">
        ...
        <section class="bg-slate-600 px-3 pb-3 rounded mb-3">
            <h2 class="mb-1 pt-2 text-lg font-medium text-gray-900">Product Basic Detail</h2>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-y-2 gap-x-4">
                <div>
                    <label class="text-white">Product Category</label>
                    <select name="category_id"
                        class="w-full bg-slate-600 border border-gray-700 rounded py-0.5 focus:outline-none text-white">
                ...
                <div>
                    ...
                    <input type="text" name="title" value="{{ $data->title }}" placeholder="Enter Product Name/Title"
                        class="w-full bg-transparent border border-gray-700 rounded py-0.5 px-2 focus:outline-none text-white">
                </div>
        ...
        <section id="product_variants" class="bg-slate-600 px-3 pb-3 rounded mb-3">
            <h2 class="mb-1 pt-2 text-lg font-medium text-gray-900">Product Variant Detail</h2>
            ...
            @foreach ($data->variant as $variantItem)
                ...
                <div class="flex justify-between gap-3 mb-2 border-b border-gray-400 pb-2">
                    <div class="grid grid-cols-1 md:grid-cols-5 gap-3">
                        ...
                        <div>
                            ...
                            <select name="color_id[]"
                                ...
                                @foreach ($colors as $item)
                                    <option value="{{ $item->id }}" @selected($item->id == $variantItem->color_id)>{{ $item->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="size_id[]"
                                ...
                                @foreach ($sizes as $item)
                                    <option value="{{ $item->id }}" @selected($item->id == $variantItem->size_id)>{{ $item->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            <label class="text-white">Stock</label>
                            <input type="number" name="stock[]" value="{{ $variantItem->stock }}"
        ...
        {{-- Product Image --}}
        <section class="bg-slate-600 px-3 pb-3 rounded mb-3">
            <h2 class="mb-1 pt-2 text-lg font-medium text-gray-900">Product Image (800x1200px ot 2:3) </h2>
            <div id="image_container" class="grid grid-cols-1 md:grid-cols-8 gap-3">
                @foreach ($data->image as $item)
                    <input type="hidden" name="image_ids[]" value="{{ $item->id }}" required>
                    <div class="relative">
                        <label for="img-1"
                            class="flex items-center justify-center bg-white rounded-md shadow-md p-1 cursor-pointer">
                            <input type="file" id="img-1" name="images[]" onchange="setImagePreview(this, event)"
                                accept="image/*" class="hidden">
                            <img src="{{ asset('storage/' . $item->path) }}" class="rounded-md aspect-[2/3] object-cover"
                                alt="">
                        </label>
                        <span onclick="deleteImage(this)"
                            class="absolute top-1 right-1 cursor-pointer w-7 h-7 flex items-center justify-center bg-white hover:bg-red-500 bg-opacity-25 hover:opacity-100 text-red-500 hover:text-white duration-300 shadow rounded-full">
                            <i class="bx bx-trash text-xl"></i>
                        </span>`
                    </div>
                @endforeach
        ...
        <button class="bg-indigo-500 text-center text-white font-medium w-full py-1 rounded shadow-md uppercase">
            Update Product
        </button>
    </form>
